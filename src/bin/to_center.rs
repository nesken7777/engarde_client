...
impl MyStateAlg {
    ...
    fn my_position(&self) -> u8 {
        use PlayerID::{One, Zero};
        match self.id {
            Zero => self.p0_position,
            One => self.p1_position,
        }
    }
}

fn distance_from_center_moved(id: PlayerID, position: u8, movement: Movement) -> u8 {
    use Direction::{Back, Forward};
    use PlayerID::{One, Zero};
    let position_i8 = i8::try_from(position).expect("絶対に起こらない");
    let card_i8 = i8::try_from(movement.card().denote()).expect("絶対に起こらない");
    match id {
        Zero => match movement.direction() {
            Forward => 12_i8.abs_diff(position_i8 + card_i8),
            Back => 12_i8.abs_diff(position_i8 - card_i8),
        },
        One => match movement.direction() {
            Forward => 12_i8.abs_diff(position_i8 - card_i8),
            Back => 12_i8.abs_diff(position_i8 + card_i8),
        },
    }
    ...
    actions.sort_unstable_by(|action1, action2| match action1 {
        Action::Move(movement1) => match action2 {
            Action::Move(movement2) => {
                distance_from_center_moved(state.id, state.my_position(), *movement1).cmp(
                    &distance_from_center_moved(state.id, state.my_position(), *movement2),
                )
            }
